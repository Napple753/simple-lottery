...
function loadSampleProgram(){
  fetch("/sample_setting.jsonc").then(res=>res.text()).then(res=>loadJSONCText(res));
}
    ...
    <div class="programPreview">
      ...
      <ul>
        <li v-for="program in settings?.program">
          <template v-if="program.type=='MESSAGE'">{{ program.message }}</template>
          <template v-if="program.type=='PRIZE'">{{ program.prize_name }} (当選者{{ program.winner_number }}人)</template>
          <template v-if="program.type=='DISPLAY_WINNERS'">(当選者一覧表示)</template>
        </li>
      </ul>
    </div>
